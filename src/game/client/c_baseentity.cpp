...
{
	if ( C_BaseEntity::sm_bAccurateTriggerBboxChecks && pCollide->GetSolid() == SOLID_BBOX )
	{
...
{
public:
	CTriggerMoved_ClientSide() : m_TouchedEntities( 8, 8 )
	{

	}

	void TriggerMoved( C_BaseEntity *pTriggerEntity )
	{
	...
	IterationRetval_t EnumElement( IHandleEntity *pHandleEntity )
	{
		...
		//Msg("vecWorldMins: %f %f %f\n", vecWorldMins.x, vecWorldMins.y, vecWorldMins.z);
		//Msg("vecWorldMaxs: %f %f %f\n", vecWorldMaxs.x, vecWorldMaxs.y, vecWorldMaxs.z);

		IClientUnknown *pUnk = static_cast< IClientUnknown* >( pHandleEntity );
		Assert( pUnk );

		// Convert the user ID to and edict_t*...
		C_BaseEntity* pTouch = pUnk->GetBaseEntity();
		Assert( pTouch );
...
private:
	C_BaseEntity* m_pTriggerEntity;
	ICollideable* m_pTrigger;
	int m_triggerSolidFlags;
	Vector m_vecDelta;
	CUtlVector< C_BaseEntity* > m_TouchedEntities;
};

void TriggerMoved( C_BaseEntity *pTriggerEnt )
{
	CTriggerMoved_ClientSide triggerEnum;
...
{
public:
	CTouchLinks_ClientSide( C_BaseEntity *pEnt, const Vector* pPrevAbsOrigin ) : m_TouchedEntities( 8, 8 )
	{
		m_pEnt = pEnt;
	...
	IterationRetval_t EnumElement( IHandleEntity *pHandleEntity )
	{
		...
		C_BaseEntity *pTriggerEntity = pUnk->GetBaseEntity();
...
void SolidMoved( C_BaseEntity *pTriggerEnt, ICollideable *pSolidCollide, const Vector* pPrevAbsOrigin )
{
	if (!pPrevAbsOrigin)
	{
		CTouchLinks_ClientSide touchEnumerator(pTriggerEnt, NULL);

		Vector vecWorldMins, vecWorldMaxs;
		pSolidCollide->WorldSpaceSurroundingBounds( &vecWorldMins, &vecWorldMaxs );

		partition->EnumerateElementsInBox( PARTITION_CLIENT_TRIGGER_ENTITIES,
	...
	{
		CTouchLinks_ClientSide touchEnumerator(pTriggerEnt, pPrevAbsOrigin);

		// A version that checks against an extruded ray indicating the motion
		partition->EnumerateElementsAlongRay( PARTITION_CLIENT_TRIGGER_ENTITIES,
